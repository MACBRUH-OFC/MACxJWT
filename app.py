...
version = requests.get('https://raw.githubusercontent.com/koolpega/Free-Fire/refs/heads/main/version').text.strip()
...
def guest_token(uid, password):
    ...
    headers = {"Host": "100067.connect.garena.com","Content-Type": "application/x-www-form-urlencoded"}
...
def get_jwt_token_ind():
    ...
    if response:
        ...
        return jsonify({"token": decoded_response.token})

    return jsonify({"error": "Login Failed, Something went Wrong"}), 500
