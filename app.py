...
# ----------------------------------------
# AES Encryption (same as your code)
# ----------------------------------------
...
# ----------------------------------------
# STEP 2 — MajorLogin (JWT generator)
# ----------------------------------------
def MajorLogin(access_token, open_id, version):
    ...
    headers = {
        ...
        "X-GA": "v1 1",
        "ReleaseVersion": version  # dynamic version
    }
...
def get_jwt_token():
    ...
    try:
        ...
        if not access_token:
            return jsonify({"status": "error", "message": "guest login failed"})
